...
public:
    ...
    int _samples;
    float _c;
    vector<vector<float> > _per; //per[classes][4]
    ...
    Confusion(int classes, int samples) : _classes(classes), _samples(samples),
                                          _per(classes, vector<float>(4)), _ind(classes, vector<string>(classes)),
                                          _cm(classes, vector<int>(classes)) { }

    Confusion(vector<vector<float> > targets, vector<vector<float> > outputs) {
        confusion(targets, outputs);
    }

    Confusion(vector<int> targets, vector<int> outputs) {
        vector<vector<float> > tar;
        vector<vector<float> > out;
        convertToBooleanMatrix(targets, outputs, tar, out);
        confusion(tar, out);
    }

    void convertToBooleanMatrix(vector<int> targets, vector<int> outputs, vector<vector<float> > &tar,
                                vector<vector<float> > &out) {
        int numClasses =
                *max_element(targets.begin(), targets.end()) - *min_element(targets.begin(), targets.end()) + 1;
        int numSamples = targets.size();
        vector<vector<float> > t(numClasses, vector<float>(numSamples));
        vector<vector<float> > o(numClasses, vector<float>(numSamples));
    ...
    void confusion(vector<vector<float> > targets, vector<vector<float> > outputs) {
        ...
        float c = (float) count / (float) (2 * numSamples);
        ...
        // Percentages
        vector<vector<float> > per(numClasses, vector<float>(4));
        for (int row = 0; row < numClasses; row++) {
